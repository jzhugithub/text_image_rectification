...
#define BaselineNumber 2//行长度计算基准行
...
int main( )
{
	...
	cout<<"图片列数："<<origImage_o.cols<<endl;//2445
	cout<<"图片行数："<<origImage_o.rows<<endl;//3264

	//【3】转化为灰度图、均衡化
	cvtColor( origImage_o, grayImage_o, CV_BGR2GRAY );//转换为灰度图
	...
	do 
	{
		HoughLinesP(medianBlurImage_o_e, vlines, 1, CV_PI/360, medianBlurImage_o_e.rows*0.1, medianBlurImage_o_e.rows*0.3, medianBlurImage_o_e.rows*0.006 );//进行霍夫变换
	...
	//【7】依次在图中绘制出每条线段
	origImage_o.copyTo(lineateImage_o);//将缩放图复制给效果图
	...
	rotatedIm = RotateImage(origImage_o,vlin);//定义旋转类
	...
	if (disp_flag1 ==1)
	{
		ZjMethod::showAndSaveImage(origImage_o,"【1原始图】","../0image_and_header/");//显示原始图像
	...
	//【3】进行形态学滤波
	unit = int(floodFillImage_r.rows*0.00025);//与图片高度相关联的单位长度
	if (unit == 0)
	{
		unit = 1;
	}
		//cout<<"unit:"<<unit<<endl;
	dilate(floodFillImage_r,morphologyImage_r,getStructuringElement(MORPH_RECT, Size(70*unit, 2*unit)));//膨胀
	erode(morphologyImage_r,morphologyImage_r,getStructuringElement(MORPH_RECT, Size(1*unit, 7*unit)));//腐蚀

	//【4】进行细化
	ZjMethod::thin(morphologyImage_r,thinImage_r,100*unit);
	...
	//-----------------------------------【行非线性校正部分-寻找水平基准线与矫正】----------------------------------------
	//		描述：根据提取的行中心线寻找水平基准线并进行水平矫正
	//-----------------------------------------------------------------------------------------------
	//【1】变量定义
	Mat lineres_r_e,				//扩展后的行提取二值图像
		horizontalBaselineImage_r,	//水平基准线提取彩色图像
		horAdjustImage_h,			//水平调整后的彩色图像
		lineres_h,					//水平调整后行提取二值图像
		horAdjustWithLineImage_h;	//水平调整后含行提取的彩色图像
	vector<Vec4i> hlines;//为了找水平基准线，定义一个矢量结构lines2用于存放得到的线段矢量集合(列，行，列，行)
	Vec4i hlin;			//旋转图像的水平基准线段
	HorBaseLine hbasel;//寻找水平基准线的类
	HorAdjustImage	horAdjustIm,	//图像水平调整类
					horAdjustIm_t;	//二值图像水平调整类
	vector<vector<Point3f>> RES_h;//水平矫正图像行中心线
	...
	//【5】对*旋转彩色图像与行提取二值图像*进行仿射变换
	hlin = hbasel.baseSegment();//找到旋转图像的水平基准线段
	horAdjustIm = HorAdjustImage(rotatedImage_r,hlin);//定义水平调整类
	horAdjustIm_t = HorAdjustImage(lineres_r,hlin);//定义水平调整类
	horAdjustImage_h = horAdjustIm.trans();//作仿射变换
	lineres_h = horAdjustIm_t.trans();//作仿射变换

	//【6】行中心线重新提取
	RES_h=jlmethod::findline(lineres_h);

	//【7】显示图像
	horAdjustImage_h.copyTo(horAdjustWithLineImage_h);
	jlmethod::showline(horAdjustWithLineImage_h,RES_h,Scalar(0,255,255));//将行提取结果画到原水平调整后的彩色图像上

	jlmethod::showline(lineres_h,RES_h,150);//将行提取结果画到图像上
	...
	if (disp_flag4 ==1)
	{
		...
		ZjMethod::showAndSaveImage(horAdjustImage_h,"【4水平调整后的彩色图像】","../0image_and_header/");//显示水平调整后的彩色图像
		ZjMethod::showAndSaveImage(lineres_h,"【4水平调整后行提取二值图像】","../0image_and_header/");//显示水平调整后行提取图像
		ZjMethod::showAndSaveImage(horAdjustWithLineImage_h,"【4水平调整后行提取彩色图像】","../0image_and_header/");//显示水平调整后行提取彩色图像
	}
	cout<<"正在继续，请稍等"<<endl;
	waitKey(0);
	...
	//【2】去除行中心线两端干扰段
	RES_h_c = jlmethod::cutEnds(RES_h,lineres_h.cols * 0.02);

	//【3】去除干扰端后进行滤波
	RES_h_c_s = ZjMethod::smooth(RES_h_c,lineres_h.cols*0.02);

	//【4】扩展为标准的行中心线
	minX = RES_h[0][0].x;
	maxX = RES_h[0][RES_h[0].size()-1].x;
	for (int i = 1;i<RES_h.size();i++)
	{
		if (RES_h[i][0].x < minX)//找minX
		{
			minX = RES_h[i][0].x;
		}
		if (RES_h[i][RES_h[i].size()-1].x > maxX)//找maxX
		{
			maxX = RES_h[i][RES_h[i].size()-1].x;
		}
	}
	...
	//【5】中心线扩展端二次函数插值
	RES_h_i = jlmethod::lineInterpolation(RES_h_e,lineres_h.cols * 0.05);

	//【6】计算模型信息
	RES_mod_h = ZjMethod::modeling(RES_h_i,lineres_h.rows*Kpaperheight,horAdjustIm.outpulLineRow,CameraHeight,PaperHeight);
		//cout<<"RES_h.size() "<<RES_h.size()<<endl;
		//cout<<"lineres_h.rows*0.8 "<<lineres_h.rows*0.8<<endl;
		//cout<<"horAdjustIm.outpulLineRow "<<horAdjustIm.outpulLineRow<<endl;

	//【7】模型信息滤波
	RES_mod_h_s = ZjMethod::smooth(RES_mod_h,lineres_h.cols*0.02);
	...
	//【9】矫正页面
	correctedImage = Mat(horAdjustImage_h.rows,horAdjustImage_h.cols*1.5,horAdjustImage_h.type(),Scalar(255,255,255));//创建空白图像
	ZjMethod::paperCorrection(horAdjustImage_h,correctedImage,RES_leng_h);

	//【10】显示图像
	modImage_h = Mat(lineres_h.rows,lineres_h.cols,CV_8U,Scalar(0));//创建空白图像
	jlmethod::showline(modImage_h,RES_mod_h_s,255);//将行提取结果画到空白图像上

	horAdjustWithLineImage_h.copyTo(cutAndSmoothImage);
	...
	if (disp_flag5 ==1)
	{
		ZjMethod::showAndSaveImage(modImage_h,"【5滤波后模型二值图像】","../0image_and_header/");//显示模型图像
